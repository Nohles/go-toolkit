...
func TestAudioParserAcceptsNilMediaTypes(t *testing.T) {
...
func TestAudioParserInfersMediaTypeAndNaturalSortsTracks(t *testing.T) {
	...
	assert.Equal(t, &mediatype.MP4, pub.Manifest.ReadingOrder[0].MediaType)
	assert.Equal(t, &mediatype.MP4, pub.Manifest.ReadingOrder[1].MediaType)
}

func TestAudioParserEmitsPublicationRelativeHrefs(t *testing.T) {
	a := staticAsset{mediaType: mediatype.Binary}
	f := staticLinksFetcher{
		links: manifest.LinkList{{
			Href: manifest.MustNewHREFFromString("/J.K.%20Rowling%20-%20The%20Ickabog.m4b", false),
		}},
	}
	...
	pub := builder.Build()
	require.Len(t, pub.Manifest.ReadingOrder, 1)
	assert.Equal(t, "J.K.%20Rowling%20-%20The%20Ickabog.m4b", pub.Manifest.ReadingOrder[0].Href.String())
	assert.Equal(t, &mediatype.MP4, pub.Manifest.ReadingOrder[0].MediaType)
}
...
func TestAudioErrorsWhenNoAudioFile(t *testing.T) {
	...
	// actual audio track, so the parser rejects the asset and lets other parsers try.
